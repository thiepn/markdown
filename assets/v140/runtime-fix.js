...
async function ready(){for(let i=0;i<200&&!window.__V140_MASTERY__;i++)await sleep(40);const api=window.__V140_MASTERY__;if(!api)return;
 function ensureBar(){const p=$('#v130Practice'),a=api.getState().activeReview;if(!p||!a||p.dataset.lab!==a.lab)return false;let bar=$('#v140ReviewBar');if(!bar){bar=document.createElement('div');bar.id='v140ReviewBar';bar.className='v140-review';p.append(bar)}bar.innerHTML=`<div><b>${a.mode==='review'?'REVIEW SESSION':'MASTERY CONTINUATION'} · Lab ${a.lab} · ${a.level}</b><small>Do the exercise in the real workbench, then verify it through the existing v1.3 grader.</small></div><button id="v140Verify" type="button">Verify review</button>`;return true}
 ...
 const observer=new MutationObserver(()=>{if(api.getState().activeReview)queueMicrotask(ensureBar)});observer.observe(document.body,{subtree:true,childList:true});
 window.__V140_RUNTIME_FIX__={version:'1.4.0',reviewTargetWait:true,reviewBridgePersistence:true,singleCheckTelemetry:true};
}
